...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        const dir = path.join(__dirname, '../../public/temp/contributeuploads');
...
router.post('/', upload.array('files'), async (req, res) => {
    try {
        const { name, email, phone, articleTitle, articleIdea } = req.body;

        if (!name || !email || !phone || !articleTitle || !articleIdea || req.files.length === 0) {
        ...
        const mailOptions = {
            ...
            subject: `New Article Contribution: ${articleTitle}`,
            html: `
                ...
                <p><strong>Article Idea:</strong><br>${articleIdea}</p>
            `,
